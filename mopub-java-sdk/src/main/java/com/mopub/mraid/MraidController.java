...
public class MraidController {
    ...
    private final MraidBridgeListener mMraidBridgeListener = new MraidBridgeListener() {
        ...
        public void onPlayVideo(@NonNull final URI uri) {
            if(!mAdConfiguration.isPostitial()){
                handleShowVideo(uri.toString());
                Log.d("postitial", "mraidcontroller videoplayed this shouldn't hit for postitial");
            }else{
                mMraidListener.onVideoPlayed(uri.toString());
            }
        }
    };
    ...
    private ViewGroup getRootView() {
        if (mRootView == null) {           
            ...
            if (VERSION.SDK_INT >= VERSION_CODES.KITKAT) {
                Preconditions.NoThrow.checkState(mDefaultAdContainer.isAttachedToWindow());                
            }
    ...
    public void setRootViewGroup(ViewGroup holder) {
        mRootView = holder;       
    }
